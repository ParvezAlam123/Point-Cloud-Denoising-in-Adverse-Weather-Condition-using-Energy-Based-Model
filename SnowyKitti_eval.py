...
model_UNet.load_state_dict(torch.load("model_state_unet_final.pth", map_location=device))
...
def filter_and_segment_point_cloud(points, label_array, bounds, threshold=1):
    """
    ...
    filtered_labels = label_array[(points[:, 0] >= xmin) & (points[:, 0] <= xmax) &
                             (points[:, 1] >= ymin) & (points[:, 1] <= ymax)]
    
    
    pred_labels = np.ones(len(filtered_points), dtype=np.int32) 
    ...
    return result_points, comp_points, TP, FP, TN, FN 
...
SnowyKitti_path = "/media/parvez/One_Touch/snowyKITTI/dataset/sequences" 
    ...
    if sequences[seq] == "15":
       if 'info.txt' in data:
               ...
               for frame in range(len(snow_velodyne)):
                   ...
                   N_snow, _ = snowy_points.shape 
                
                   normal_points = points[label == 0]
                   N_normal, _ = normal_points.shape 
                   ...
                   filtered_snow_points = snowy_points[range_mask]  

                   range_cal = np.sqrt(normal_points[:, 0]**2+normal_points[:, 1]**2+normal_points[:, 2]**2) 
                   range_mask = range_cal >= radius_threshold 

                   filtered_normal_points = normal_points[range_mask] 

                   light_TP = light_TP + (N_snow - filtered_snow_points.shape[0])
                   light_FP = light_FP + (N_normal - filtered_normal_points.shape[0]) 

                   normal_label = np.zeros(filtered_normal_points.shape[0]) 
                   snow_label = np.ones(filtered_snow_points.shape[0]) 

                   label_array = np.hstack((normal_label, snow_label))
                   points = np.vstack((filtered_normal_points, filtered_snow_points)) 
                   ...
                   points_array = points #filtered_points   
                   ...
                   filtered_points, comp_points, light_TP_, light_FP_, light_TN_, light_FN_ = filter_and_segment_point_cloud(points, label_array, bounds, threshold=10) 
                   
                   print("time = ", time.time() - start_time)
                   light_TP = light_TP + light_TP_ 
                   light_FP = light_FP + light_FP_ 
                   light_TN = light_TN + light_TN_ 
                   light_FN = light_FN + light_FN_""" 
...
print("precision = ", light_TP / (light_TP + light_FP)) 
print("recall = ", light_TP / (light_TP + light_FN)) 
print("accuracy = ", (light_TP+light_TN) /(light_TP+light_TN+light_FP+light_FN))
